...
HandshakeEncryption::initialize_decrypt(const char* origHash, bool incoming) {
  ...
  m_info.set_decrypt(RC4(reinterpret_cast<const unsigned char*>(hash), 20, 0));
...
HandshakeEncryption::initialize_encrypt(const char* origHash, bool incoming) {
  ...
  m_info.set_encrypt(RC4(reinterpret_cast<const unsigned char*>(hash), 20, 1));
...
HandshakeEncryption::encrypt_vc_to_sync(const char* origHash) {
  ...
  auto peerEncrypt = RC4(reinterpret_cast<const unsigned char*>(hash), 20, 1);
