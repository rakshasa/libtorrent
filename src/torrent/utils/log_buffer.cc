...
log_buffer::find_older(int32_t older_than) {
  ...
  return std::find_if(begin(), end(), [older_than](const auto& log) { return log.is_younger_or_same(older_than); });
}
...
log_open_log_buffer(const char* name) {
  ...
  log_open_output(name, [b = buffer.get()](auto data, auto len, auto group) { b->lock_and_push_log(data, len, group); });
  return buffer;
}
