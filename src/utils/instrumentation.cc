...
std::array<std::atomic<int64_t>, INSTRUMENTATION_MAX_SIZE> instrumentation_values lt_cacheline_aligned;

inline int64_t
instrumentation_fetch_and_clear(instrumentation_enum type) {
#ifdef LT_INSTRUMENTATION
  // Unlike most assignments, atomics don't return the original value,
  // but the stored value, so we have to copy it first
  auto value                   = instrumentation_values[type].load();
  instrumentation_values[type] = 0;
  return value;
#else
  return 0;
#endif
...
#else
  lt_log_print(LOG_INSTRUMENTATION_POLLING, "rTorrent not compiled with intrumentation support");
  lt_log_print(LOG_INSTRUMENTATION_TRANSFERS, "rTorrent not compiled with intrumentation support");
#endif
}
