...
UdnsResolver::resolve(void* requester, const std::string& hostname, int family, resolver_callback&& callback) {
  ...
  if (family == AF_INET || family == AF_UNSPEC) {
    ...
    if (query->a4_query == nullptr) {
      LT_LOG_REQUESTER("malformed A query : name:%s", hostname.c_str());
  ...
  if (family == AF_INET6 || family == AF_UNSPEC) {
    ...
    if (query->a6_query == nullptr) {
      LT_LOG_REQUESTER("malformed AAAA query : name:%s", hostname.c_str());
  ...
  LT_LOG_REQUESTER("resolving : name:%s family:%d", hostname.c_str(), family);
...
UdnsResolver::cancel(void* requester) {
  ...
  LT_LOG_REQUESTER("canceled : queries:%d malformed:%d", query_count, malformed_count);
}
...
UdnsResolver::flush() {
  ...
  for (auto& malformed : malformed_queries) {
    auto query = malformed.second.get();
...
bool
UdnsResolverInternal::handle_cname_recursion(::dns_ctx *ctx, UdnsQuery* query, const char* cname, bool is_ipv6) {
  if (cname == nullptr) return false;

  if (++query->cname_depth > 8) {
    query->error_sin = EAI_FAIL; // Or error_sin6 for IPv6
    LT_LOG_QUERY("CNAME depth exceeded : original:%s current:%s", query->requester, query->hostname.c_str());
    return false; 
  }

  query->hostname = cname;
  LT_LOG_QUERY("CNAME encountered, retrying : original:%s cname:%s depth:%d", query->requester, query->hostname.c_str(), query->cname_depth);

  if (is_ipv6) {
    query->a6_query = ::dns_submit_a6(ctx, query->hostname.c_str(), 0, &UdnsResolverInternal::a6_callback_wrapper, query);
  } else {
...
UdnsResolverInternal::a4_callback_wrapper(::dns_ctx *ctx, ::dns_rr_a4 *result, void *data) {
  ...
  // 2. Handle CNAME Recursion (via helper)
  const char* cname = (result != nullptr) ? result->dnsa4_cname : ::dns_cname(ctx);
  if (handle_cname_recursion(ctx, query, cname, false)) {
    return; // New query submitted, wait for next callback
  }

  // 3. Handle Final Error (No result and no valid CNAME to follow)
  query->error_sin = udnserror_to_gaierror(::dns_status(ctx));
  LT_LOG_QUERY("No A records found : name:%s error:'%s'",  query->hostname.c_str(), gai_strerror(query->error_sin));
...
UdnsResolverInternal::a6_callback_wrapper(::dns_ctx *ctx, ::dns_rr_a6 *result, void *data) {
  ...
  if (query->deleted) {
    LT_LOG_QUERY("AAAA records received, but query was deleted : name:%s", query->hostname.c_str());
  ...
  if (result == nullptr || result->dnsa6_nrr == 0) {
    ...
    LT_LOG_QUERY("no AAAA records received, calling back with error : name:%s error:'%s'",
                 query->hostname.c_str(), gai_strerror(query->error_sin6));

  } else {
    query->result_sin6 = sin6_make();
    query->result_sin6->sin6_addr = result->dnsa6_addr[0];

    LT_LOG_QUERY("AAAA records received : name:%s nrr:%d", query->hostname.c_str(), result->dnsa6_nrr);
  }
...
UdnsResolver::process_partial_result_unsafe(query_map::iterator itr) {
  ...
  if (query->a4_query != nullptr || query->a6_query != nullptr) {
    LT_LOG_QUERY("processing results, waiting for other queries : name:%s", query->hostname.c_str());
    return;
  }
  ...
  tmp->deleted = true;
  tmp->parent->m_queries_unsafe.erase(itr);

  process_final_result_unsafe(std::move(tmp));
}

void
UdnsResolver::process_final_result_unsafe(std::unique_ptr<UdnsQuery>&& query) {
  if (query->canceled) {
    LT_LOG_QUERY("processing results, canceled : name:%s", query->hostname.c_str());
    return;
  }
