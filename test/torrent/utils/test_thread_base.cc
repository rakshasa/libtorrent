...
test_thread_base::test_lifecycle() {
  ...
  CPPUNIT_ASSERT(thread->state() == torrent::ThreadBase::STATE_UNKNOWN);
  CPPUNIT_ASSERT(thread->test_state() == test_thread::TEST_NONE);

  thread->init_thread();
  CPPUNIT_ASSERT(thread->state() == torrent::ThreadBase::STATE_INITIALIZED);
...
test_thread_base::test_global_lock_basic() {
  ...
  CPPUNIT_ASSERT(torrent::ThreadBase::global_queue_size() == 0);
